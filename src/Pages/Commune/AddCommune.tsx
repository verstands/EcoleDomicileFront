...
const AddCommune = () => {
    const [data, setdata] = useState<CommuneInterface>({
        nom: "",
        id_ville: ""
    });
    const [dataPays, setDataPays] = useState<VilleInterface[] | null>(null);
    ...
    const getville = async () => {
        try {
            const response = await villeService.getVille() as PaysInterface;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setLoading(true);
        const response = await communeService.postCommune(data);
        setLoading(false);
        toast.success('commune a été creé avec success');
        try {
            setLoading(false);
        } catch (error: unknown) {
        ...
        <div className="px-10">
            <div className="border-white m-3  bg-white p-10 rounded-[10px] shadow">
                <form onSubmit={handleSubmit}>
                    <h2 className="text-[20px] font-bold">Ajouter une commune</h2>
                    <br />
                    <hr />
                    <br />
                    <div>
